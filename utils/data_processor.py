...
class WeatherDataProcessor:
    ...
    def process_temperature_data(self, df):
        ...
        Args:
            df (pd.DataFrame): Raw temperature data
            
        Returns:
            pd.DataFrame: Processed temperature data
        """
        if df is None:
            return None
            
        try:
            # Assuming temperature might come in Kelvin and needs conversion to Celsius
            ...
            # Add min/max if data spans multiple days
            if 'time' in df.columns:
                ...
                # Merge the stats back if useful
